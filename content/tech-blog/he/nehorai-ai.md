...
בוט AI שנותן מחירי טיסות של אתמול הוא חסר ערך. אבל לקרוא ל-API של מחירים בזמן אמת במהלך שיחת צ'אט אומר 3-5 שניות זמן תגובה: אף אחד לא מחכה כל כך הרבה זמן בצ'אט.
...
ההחלטה הארכיטקטונית הכי חשובה: **אף פעם לא לקרוא ל-API של מחירים בזמן אמת במהלך צ'אט**. במקום זאת, סורקים מתוזמנים רצים על Cloudflare Workers Cron כל 30 דקות, סורקים דילי טיסות מ-SecretFlights, לוחות שיעורי תורה, רשימות הופעות, תוצאות ספורט מ-Football API, וחדשות מחמישה ערוצי טלגרם.
...
flowchart TD
    ...
    CRAWL --> DEALS["דילים: סריקת SecretFlights ל-17 יעדים"]
    CRAWL --> TORAH["תורה: לוחות שיעורים"]
    CRAWL --> SPORTS["ספורט: תוצאות Football API"]
...
גם עם הגרף, ה-pipeline המלא לוקח 2-4 שניות. זה נצח ב-UI של צ'אט. אז הקליינט שולח שתי בקשות במקביל:
...
המשתמש רואה את התשובה המהירה תוך ~300ms, ואז התשובה המלאה מחליפה אותה 2-3 שניות אחר כך. זה מרגיש מיידי.
...
NehorAI גם מנהל ערוץ טלגרם שמשדר סיכומי חדשות. כל 30 דקות (בשעות פעילות, 8:00-20:00 שעון ישראל, בשעות זוגיות), הסורק גורד חמישה ערוצי חדשות ישראליים בטלגרם, מסנן פריטים מהשעה האחרונה, ומעביר אותם דרך Gemini עם הפרסונה של נהורAI כדי לייצר סיכום חדשות בסלנג שמתפרסם בערוץ הטלגרם של הבוט.
